...
//   A) a FUTURE re-add of phase_custom_name on an already-migrated DB (user_version=1) must NOT trigger
//      the destructive rebuild — the exact landmine that Phase-5B's contract_service_id re-add caused.
...
init();
assert.strictEqual(userVersion(), 1, 'init() stamps the schema-version marker (user_version=1)');
db.prepare("INSERT INTO users(id,username,display_name,password_hash) VALUES (1,'owner','Owner','h')").run();
for (let i = 1; i <= 3; i++) {
  db.prepare('INSERT INTO cash_out(amount_paise,tx_date,by_type,by_user_id,ledger_code,contract_scope,tenant_id) VALUES (?,?,?,?,?,?,?)')
    .run(1000 * i, '2025-04-0' + i, 'user', 1, '4.0', 'extra', 1);
}
const base = fp();
assert.strictEqual(base.n, 3, 'seed: 3 rows'); assert.strictEqual(base.s, 6000, 'seed: sum 6000');

// 2) SCENARIO A — a re-add of phase_custom_name on a MARKED DB (user_version=1) must be left alone.
...
assert.strictEqual(userVersion(), 1, 'B: marker re-stamped after the rebuild');
