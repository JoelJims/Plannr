// Loan interest recorded under ledger 22.0 / sub-ledger 22.5 (Loan interest) as an ordinary cash_out
...
let A;
const interestBody = { amountRupees: '5000.00', txDate: '2026-07-15', byType: 'user', ledgerCode: '22.0', subledgerCode: '22.5', contractScope: 'extra' };
...
test('the create route accepts ledger 22.0 + sub-ledger 22.5 (Loan interest)', async () => {
  const r = await addInterest();
  assert.strictEqual(r.status, 201, JSON.stringify(r.json));
  assert.strictEqual(r.json.entry.ledgerCode, '22.0');
  assert.strictEqual(r.json.entry.subledgerCode, '22.5');
  assert.strictEqual(r.json.entry.contractScope, 'extra');
});

test('interest counts in total spend + the 22.0 finance slice, kept separate from build ledgers, and NEVER in owed (F)', async () => {
  const cid = H.seedContract({ pricePaise: 10000000 }); // stated ₹1,00,000
  H.seedPayment({ contractId: cid, amountPaise: 4000000 }); // paid ₹40,000
  H.seedCashOut({ amountPaise: 3200000, byUserId: A.user.id, ledgerCode: '6.0', subledgerCode: '6.1', contractScope: 'extra' }); // ₹32,000 cement (build)
  await addInterest(); // ₹5,000 interest under 22.0/22.5
  ...
  const fin = o.ledgers.find((L) => L.code === '22.0');
  assert.ok(fin && fin.totalPaise === 500000, '22.0 finance slice carries only the interest');
  const sub = fin.subs.find((s) => s.code === '22.5');
  assert.ok(sub && sub.totalPaise === 500000, '22.5 sub-slice = the interest');
  const build = o.ledgers.find((L) => L.code === '6.0');
...
test('a backup round-trips a 22.5 interest row (import validation accepts it)', async () => {
  await addInterest();
  const backup = (await H.get('/api/backup/export', { cookie: A.cookie })).json;
  assert.ok(backup.tables.cash_out.some((r) => r.ledger_code === '22.0' && r.subledger_code === '22.5'), 'export contains the 22.5 interest row');
  const imp = await H.post('/api/backup/import', backup, { cookie: A.cookie });
  assert.strictEqual(imp.status, 200, JSON.stringify(imp.json));
  const fin = (await overview()).ledgers.find((L) => L.code === '22.0');
