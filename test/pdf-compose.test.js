...
before(async () => { await H.startApp(); H.seedLoggedIn(); });
after(async () => { await H.stopApp(); });
beforeEach(() => { H.clearLedger(); });

test("theme:'dark' uses the dark background #15161a; light does not", () => {
  H.seedCashOut({ txDate: '2026-07-12', amountPaise: 111100 });
  const dark = H.app._overviewPdfHtml({ theme: 'dark' });
  const light = H.app._overviewPdfHtml({ theme: 'light' });
...
test("a real range excludes out-of-range rows and shows the 'Date range' label (not 'All transactions')", () => {
  H.seedCashOut({ txDate: '2026-07-12', amountPaise: 111100 }); // in range
  H.seedCashOut({ txDate: '2026-07-25', amountPaise: 222200 }); // out of range (after end)
  const html = H.app._overviewPdfHtml({ part: 'full', range: { start: '2026-07-01', end: '2026-07-20' } });
...
test("no range shows the 'All transactions' label and includes every seeded date", () => {
  H.seedCashOut({ txDate: '2026-07-12', amountPaise: 111100 });
  H.seedCashOut({ txDate: '2026-07-25', amountPaise: 222200 });
  const html = H.app._overviewPdfHtml({ part: 'full' });
