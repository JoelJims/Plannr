...
// never appears on that path — so the everyday entry keeps its Phase-4 cost. (The contract-stated
// field it used to check alongside is gone: the reimbursement offset was removed.)
// A "tap" here = one discrete pointer/selection interaction (a select choice, a
...
(async () => {
  ...
  // ---- structural proof: the picker is hidden for a NON-CONTRACT row, and the removed
  // contract-stated field is absent from the DOM entirely (not merely hidden) ----
  const statedGone = (await page.$('#contractStatedWrap')) === null && (await page.$('#contractStated')) === null;
  ...
  // session user — 0 taps each. The service picker is NOT on this path (proven above).
  let taps = 0;
  const tap = () => { taps++; };
  for (let i = 0; i < 5; i++) {
    const wantRows = i + 1;
  ...
  // A single in-contract entry that USES the picker = the same base + pick a service (1). So the
  // picker's marginal cost is +1 tap, and ONLY on an in-contract row.
  ...
  console.log('  contract-stated field removed from the DOM :', statedGone);
  console.log('  picker appears ONLY when in-contract       :', svcShownIncluded);
  ...
  const ok = svcHiddenExtra && statedGone && svcShownIncluded && rows === 5;
  await browser.close();
